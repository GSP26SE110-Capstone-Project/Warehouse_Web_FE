...
export const ManageOutbound: React.FC = () => {
        ...
        try {
            ...
            if (modalMode === 'create') {
                ...
                if (response.data.success) {
                    ...
                    // Reload contracts
                    if (warehouseId) {
                        const refreshResponse = await outboundApi.getAllOutboundRequestsByWarehouse(warehouseId)
    ...
    useEffect(() => {
        const fetchOutboundRequests = async () => {
            ...
            try {
                setLoading(true)
                setError(null)
                const response = await outboundApi.getAllOutboundRequestsByWarehouse(warehouseId)
        ...
        <div className="overflow-y-auto overflow-x-hidden p-6 md:p-8 bg-slate-50 text-slate-800">
            <div className="max-w-[1400px] mx-auto flex flex-col gap-6">
                ...
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-2xl md:text-3xl font-bold text-slate-900 mb-1">Quản lý đơn xuất kho</h1>
                        <p className="text-slate-500 text-sm">Xem xét và phê duyệt các đơn xuất kho</p>
                    </div>
                    <button
